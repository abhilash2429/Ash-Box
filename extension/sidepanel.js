...
async function checkDocker() {
  ...
  if (result.ok) {
    ...
    appendLine('[executor] Start Docker Desktop and bridge server.', 'system');
  }
}
...
function loadTheme() {
  chrome.storage.local.get(['ashBoxTheme'], (result) => {
    const theme = result.ashBoxTheme || 'dark';
...
function toggleTheme() {
  ...
  chrome.storage.local.set({ ashBoxTheme: nextTheme });
